...
class TodoRepository(BaseRepository[Todo]):
    ...
    async def get_by_id_with_tag(self, id: int) -> Optional[Todo]:
        result = await self.db.execute(
            select(Todo).where(Todo.id == id).options(selectinload(Todo.tag))  # 使用 selectinload
        )
        return result.scalar_one_or_none()
    
    async def get_all_with_tags(self, skip: int = 0, limit: int = 100) -> List[Todo]:
        result = await self.db.execute(
            select(Todo).offset(skip).limit(limit).options(selectinload(Todo.tag))  # 使用 selectinload
        )
